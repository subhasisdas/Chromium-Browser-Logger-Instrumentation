...
URLRequest::URLRequest(const GURL& url,
 ...
 if(sqlite3_prepare_v2(database,"CREATE TABLE IF NOT EXISTS DOM_EVENT(id INTEGER PRIMARY KEY AUTOINCREMENT, function TEXT, details TEXT, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP);" , -1, &statement, 0) == SQLITE_OK)
{
        ...
        std::cout << "Database Table not created!";


  char insert_sql[4096];
  snprintf(insert_sql, sizeof(insert_sql), "INSERT INTO DOM_EVENT(function, details) VALUES('DOM_EVENT_DOMTREE' , '%s');", url.GetContent().c_str() );
  ...
  std::cout << "url requested " << url.GetContent().c_str() << "\n";
