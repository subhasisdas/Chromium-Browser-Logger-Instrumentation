...
void HttpResponseHeaders::AddHeader(const std::string& header) {
  ...
  //Insert code here sdas
  
#ifdef __ANDROID__
  std::string path =  "/mnt/sdcard/chrome_log.sqlite";
#else
  struct passwd *pw = getpwuid(getuid());
  const char *homedir = pw->pw_dir;
  std::string path = std::string(homedir) + "/chrome_log.sqlite";
#endif
 ...
 if(sqlite3_prepare_v2(database,"CREATE TABLE IF NOT EXISTS http_response (id INTEGER PRIMARY KEY AUTOINCREMENT, function TEXT, details TEXT, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP);" , -1, &statement, 0) == SQLITE_OK)
...
snprintf(insert_sql, sizeof(insert_sql), "INSERT INTO http_response(function, details) VALUES('Response_Header' , '%s');",  new_raw_headers.c_str() );
...
//Insert code here sdas
  ...
  char insert_sql[4096];
  snprintf(insert_sql, sizeof(insert_sql), "INSERT INTO http_response(function, details) VALUES('HTTP_Redirection_Location' , '%s');", (*location).c_str() );
