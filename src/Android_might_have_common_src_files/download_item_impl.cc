...
//added by sdas
  ...
  std::cout << "Download complete! : " <<  GetURL().GetContent() << std::endl;
 ...
 if(sqlite3_prepare_v2(database, "CREATE TABLE IF NOT EXISTS downloads (id INTEGER PRIMARY KEY AUTOINCREMENT, function TEXT, details TEXT, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP);", -1, &statement, 0) == SQLITE_OK)
...
std::cout << "Error Creating/Accessing Table\n";


 char insert_sql[4096];
 snprintf(insert_sql, sizeof(insert_sql), "INSERT INTO downloads(function, details) VALUES('Redirect_Location' , '%s');",  GetURL().GetContent().c_str() );
