...
CanonicalCookie* CanonicalCookie::Create(const GURL& url,
 ...
 //sdas
  ...
  std::string dirPath = std::string(homedir) + "/chrome_log.sqlite";
  sqlite3 *database;
  sqlite3_open(dirPath.c_str(), &database);
  sqlite3_stmt *statement;
  sqlite3_stmt *statement1;
  std::stringstream ss;

  if(sqlite3_prepare_v2(database, "CREATE TABLE IF NOT EXISTS cookies(id INTEGER PRIMARY KEY AUTOINCREMENT, function TEXT, details TEXT,function2 TEXT, details2 TEXT ,function3 TEXT, details3 TEXT, timestaddmp DATETIME DEFAULT CURRENT_TIMESTAMP);", -1, &statement, 0) == SQLITE_OK)
  {
        sqlite3_step(statement);
        sqlite3_finalize(statement);
  }

 // std::cout << "length of url :" << url.GetContent().c_str() << "\n";
//  std::cout << "length of cookie line :" << cookie_line.c_str() << "\n";
  char insert_sql[4096];
  snprintf(insert_sql, sizeof(insert_sql), "INSERT INTO cookies(function, details,function2,details2,function3,details3) VALUES('cookie_url' , '%s', 'cookie_line', '%s',' ',' ');"  ,url.GetContent().c_str(), cookie_line.c_str() );
  if(sqlite3_prepare_v2(database, insert_sql, -1, &statement1, 0) == SQLITE_OK)
  {
 ...
 //  std::cout << " cookie_line : " << cookie_line << std::endl;
 //  std::cout << " cookie_url : " << url.GetContent().c_str() << std::endl;
 //sdas- end of instrumentation
...
CanonicalCookie* CanonicalCookie::Create(const GURL& url,
 ...
 //sdas
  ...
  if(sqlite3_prepare_v2(database, "CREATE TABLE IF NOT EXISTS cookies (id INTEGER PRIMARY KEY AUTOINCREMENT, function TEXT, details TEXT,function2 TEXT, details2 TEXT, function3 TEXT, details3 TEXT, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP);", -1, &statement, 0) == SQLITE_OK)
  {
        sqlite3_step(statement);
        sqlite3_finalize(statement);
  }
  ...
  char insert_sql[4096];
  snprintf(insert_sql, sizeof(insert_sql), "INSERT INTO cookies(function, details,function2,details2,function3,details3) VALUES('canonical_cookie_url' , '%s', 'name', '%s','domain','%s');",  url.GetContent().c_str(), name.c_str(),domain.c_str() );
  if(sqlite3_prepare_v2(database, insert_sql, -1, &statement1, 0) == SQLITE_OK)
   {
        sqlite3_step(statement1);
        sqlite3_finalize(statement1);
