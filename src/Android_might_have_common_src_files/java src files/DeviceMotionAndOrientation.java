...
class DeviceMotionAndOrientation implements SensorEventListener {
    ...
    public boolean start(int nativePtr, int eventType, int rateInMilliseconds) {
        ...
        Log.i("chromeInfo" , "Started listening for sensor " + eventType + "event at rate " + rateInMilliseconds );
	...
	Log.i("chromeInfo" , "Stopped listening for sensor " + eventType);
        synchronized (mNativePtrLock) {
            switch (eventType) {
                case DEVICE_ORIENTATION:
    ...
    public void onSensorChanged(SensorEvent event) {
   	...
   	Log.i("chromeInfo" , "Sensor changed!" );
	Log.i("chromeInfo","event type :"+event.sensor.getType()+" values: "+event.values);
    }
    ...
    void sensorChanged(int type, float[] values) {
	    ...
	    Log.i("chromeInfo" , "Accelerometer changed!" );
                if (mDeviceOrientationIsActive) {
                   getOrientationUsingGetRotationMatrix();
	   // Log.i("chromeInfo" , "Orientation changed!" );
                }
                break;
	   	 ...
	   	 Log.i("chromeInfo" , "Linear_Accelerometer changed!" );
                break;
            case Sensor.TYPE_GYROSCOPE:
                if (mDeviceMotionIsActive) {
                    gotRotationRate(values[0], values[1], values[2]);
                }
	    	 Log.i("chromeInfo" , "Gyroscope changed!" );
                break;
            case Sensor.TYPE_MAGNETIC_FIELD:
	   	  ...
	   	  Log.i("chromeInfo" , "Magnetic Field changed!" );
    ...
    protected void gotOrientation(double alpha, double beta, double gamma) {
	    ...
	    Log.i("chromeInfo" , "Got Orientation : " + alpha + " " + beta + " " + gamma );
    }
    ...
    protected void gotAcceleration(double x, double y, double z) {
	    ...
	    Log.i("chromeInfo" , "Got Acceleration!" );
    }
    ...
    protected void gotAccelerationIncludingGravity(double x, double y, double z) {
	    ...
	    Log.i("chromeInfo" , "Got Acc incl Gravity!" );
    }
    ...
    protected void gotRotationRate(double alpha, double beta, double gamma) {
	    ...
	    Log.i("chromeInfo" , "Got Rotation Rate!" );
    }
